...
export const useChecklist = () => {
  ...
  const itemCollections = useMemo(() => {
    ...
    // Exclude locked and ancestor-skipped items from progress — users can't act on them
    const progressItems = availableItems.filter((item) => !item.isLockedBy && !item.isSkipped);
    const openProgressItems = progressItems.filter((item) => item.isOpen);
    const progress = progressItems.length
      ? Math.round(
          ((progressItems.length - openProgressItems.length) / progressItems.length) * 100
        )
      : 100;
